#include <d3dcompiler.h> // For compiling shader files into bytecode
...
Text2D::Text2D(string filename, ID3D11Device* device, ID3D11DeviceContext* context)
{
	...
	hr = D3DCompileFromFile(L"Shaders/text2d_shaders.hlsl", 0, 0, "TextVS", "vs_4_0", 0, 0, &VS, &error);
	if (FAILED(hr))
	{
	...
	hr = D3DCompileFromFile(L"Shaders/text2d_shaders.hlsl", 0, 0, "TextPS", "ps_4_0", 0, 0, &PS, &error);
	if (FAILED(hr))
	{
		OutputDebugStringA(reinterpret_cast<const char*>(error->GetBufferPointer()));
		error->Release();
...
void Text2D::RenderText(void)
{
	...
	// Save current rendering states so we can revert again after rendering text
	// - NOTE: This is not the most efficient way to handle this but will simplify it for us in this case
	ID3D11VertexShader* tempVS;
	ID3D11PixelShader* tempPS;
	ID3D11InputLayout* tempIL;
	pImmediateContext->IAGetInputLayout(&tempIL);
	pImmediateContext->VSGetShader(&tempVS, 0, 0);
	pImmediateContext->PSGetShader(&tempPS, 0, 0);
	...
	// Set rendering states back to what they were before the text render
	pImmediateContext->VSSetShader(tempVS, 0, 0);
	pImmediateContext->PSSetShader(tempPS, 0, 0);
	pImmediateContext->IASetInputLayout(tempIL);
	tempVS->Release();
	tempPS->Release();
	tempIL->Release();
}
