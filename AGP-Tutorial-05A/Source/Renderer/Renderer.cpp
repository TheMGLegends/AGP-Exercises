...
void Renderer::RenderFrame()
{
	...
	// INFO: Camera Viewing
	XMVECTOR eyePosition{ camera.x, camera.y, camera.z };
	XMVECTOR lookTo{ sin(camera.yaw) * sin(camera.pitch),
					 cos(camera.pitch),
					 cos(camera.yaw)* sin(camera.pitch) };
	XMVECTOR camUp{ 0.0f, 1.0f, 0.0f }; // World Up
	view = XMMatrixLookToLH(eyePosition, lookTo, camUp);
