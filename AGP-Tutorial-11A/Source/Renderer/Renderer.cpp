...
HRESULT Renderer::InitPipeline()
{
	...
	// INFO: VS and PS for reflection
	LoadVertexShader(L"Compiled Shaders/ReflectiveVShader.cso", &pVertexShader, &pInputLayout);
	LoadPixelShader(L"Compiled Shaders/ReflectivePShader.cso", &pPixelShader);

	// INFO: VS and PS for particle
	LoadVertexShader(L"Compiled Shaders/ParticleVShader.cso", &pVSParticle, &pInputLayoutParticle);
	LoadPixelShader(L"Compiled Shaders/ParticlePShader.cso", &pPSParticle);
...
void Renderer::InitGraphics()
{
	...
	// INFO: Create Constant Buffer for particle
	cbd.ByteWidth = sizeof(CBufferParticle);
	if (FAILED(pDevice->CreateBuffer(&cbd, NULL, &pParticleCBuffer)))
	{
		OutputDebugString(L"Failed to create particle constant buffer\n");
		return;
	}
...
HRESULT Renderer::LoadParticle()
{
	...
	bd.Usage = D3D11_USAGE_DYNAMIC;
	bd.ByteWidth = sizeof(XMFLOAT3) * 6;
	bd.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bd.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr = pDevice->CreateBuffer(&bd, NULL, &pParticleVertexBuffer);
	...
	// INFO: Copy vertices into buffer
	D3D11_MAPPED_SUBRESOURCE ms = { 0 };
	pDeviceContext->Map(pParticleVertexBuffer, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &ms); // INFO: Lock the buffer
	memcpy(ms.pData, vertices, sizeof(vertices)); // INFO: Copy the data
	pDeviceContext->Unmap(pParticleVertexBuffer, NULL); // INFO: Unlock the buffer
...
void Renderer::Clean()
{
	if (pParticleCBuffer) pParticleCBuffer->Release();
	if (pInputLayoutParticle) pInputLayoutParticle->Release();
	if (pVSParticle) pVSParticle->Release();
	if (pPSParticle) pPSParticle->Release();
	if (pRasterParticle) pRasterParticle->Release();
	if (pRasterParticleSolid) pRasterParticleSolid->Release();
	if (pParticleVertexBuffer) pParticleVertexBuffer->Release();
